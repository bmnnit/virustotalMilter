...
class virustotalMilter(Milter.Milter):
  ...
  def header(self,name,val):
    ...
    if self.fp:
      strAsBytes = str.encode("%s: %s\n" % (name ,val) )
      self.fp.write(strAsBytes)	# add header to buffer
    return Milter.CONTINUE
  ...
  def isFileMalicious(self, hash):
    ...
    jsonData = json.loads ( body.decode('iso-8859-1') )
    ...
    try:
      response = jsonData['error']['code']
      if response == 'NotFoundError':
        self.log('VIRUSTOTAL not found hash: ', hash)
        return False
    except KeyError:
  ...
  def eom(self):
    ...
    self.fp.seek(0)
    #msg ist komplette email inkl header
...
if __name__ == "__main__":
  ...
  ip = config['DEFAULT']['ListenIp']
  port = config['DEFAULT']['ListenPort']
  tempDir = config['DEFAULT']['TempDir']
  default = "inet:{0}@{1}".format(port, ip)
  socketname = default
  # os.getenv("HOME") + "/pythonsock"
...
Virustotal.com milter startup""" % socketname)
  sys.stdout.flush()
